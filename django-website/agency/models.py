...
class AgencyPage(BaseModel):
    ...
    project_link = models.URLField(
        help_text=_("Your website page URL"), null=True, blank=True
    )
...
class TeamMember(Orderable):
    ...
    gravatar = models.CharField(
        max_length=150, help_text=_("Add your Gravatar email"), null=True, blank=True
    )
    photo = models.ForeignKey(
        Image, on_delete=models.CASCADE, null=True, blank=True, related_name="+"
    )
    ...
    twitter = models.URLField(
        help_text=_("Your Twitter page URL"), null=True, blank=True
    )
    gplus = models.URLField(help_text=_("Your Google+ page URL"), null=True, blank=True)
    facebook = models.URLField(
        help_text=_("Your Facebook page URL"), null=True, blank=True
    )
    linkedin = models.URLField(
        help_text=_("Your LinkedIn page URL"), null=True, blank=True
    )

    medium = models.URLField(help_text=_("Your Medium page URL"), null=True, blank=True)
    behance = models.URLField(
        help_text=_("Your Behance page URL"), null=True, blank=True
    )
    dribbble = models.URLField(
        help_text=_("Your Dribble page URL"), null=True, blank=True
    )
